...
namespace General
{
    ...
    public class RScriptRunner
    {
        ...
        string UserSpecificFolder = null;
        string RscriptexePath = null;
        ...
        public static void SetRScriptPath(string path)
        {
            ...
            /*             if (File.Exists(path) && path.Contains(".exe"))
                        {
                            bool hasChanged = path != RscriptexePath;
        ...
        public static string RunFromCmd(string rCodeFilePath, string args, bool writeOut)
        {
            ...
            try
            {
                ...
                HelperFunctions.LogDiagnosticsProcess(proc, writeOut);
        ...
        static void GeneProcHandler(object sender, DataReceivedEventArgs e, string UserSpecificFolder)
        {
            //new StreamWriter(Directory.GetCurrentDirectory() + "\\Output\\gene_log.txt", true))
            using (StreamWriter geneWriter =
                new StreamWriter(Path.Combine(UserSpecificFolder, "gene_expr.txt"), false))
            {
        ...
        public static void WriteToServer(string s, string UserSpecificFolder = "")
        {

            string inputFilePath = Path.Combine(UserSpecificFolder, "server.input.R");
        ...
        public static string RunRScript(string path, string args = "", string UserSpecificFolder = "")
        {


            serverIdle = false;
            string result = null;
            string inputFilePath = Path.Combine(UserSpecificFolder, "mainServer.input.lock");
            if (!File.Exists(inputFilePath))
            {
                ...
                {
                    SchapiroLabLog.Log(e.Source, e.Message, e.StackTrace);
